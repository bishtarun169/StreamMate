...
export default function Room() {
  ...
  const [mediaSource, setMediaSource] = useState(passedState.initialMediaSource || null); // 'youtube' | 'twitch' | 'instagram' | 'custom'
  const [activeVideo, setActiveVideo] = useState(passedState.initialMediaUrl || null); // The final video URL/ID
  ...
  const [participants, setParticipants] = useState([
    { name: `${initialNickname} (You)`, role: isCreator ? "Creator" : "Viewer", status: "online", avatar: initialNickname.charAt(0).toUpperCase() },
    { name: "Sarah Connor", role: "Viewer", status: "online", avatar: "S" },
    { name: "John Doe", role: "Viewer", status: "online", avatar: "J" },
  ]);
  ...
  return (
    <div className="h-screen w-screen bg-[#0f0f13] text-white flex flex-col font-sans transition-colors duration-300 overflow-hidden relative">
      ...
      <main className="w-full h-[calc(100vh-5rem)] flex flex-col lg:flex-row relative z-10 overflow-hidden">
        ...
        <section className={`w-full flex flex-col lg:border-r lg:border-zinc-850 max-w-full relative overflow-hidden p-2 sm:p-4 lg:p-8 transition-all duration-300 ${
          isSidebarMinimized ? "h-full flex-grow" : "h-auto lg:h-full flex-grow-0 lg:flex-grow flex-shrink-0 lg:flex-shrink"
        }`}>
          ...
          <div className="flex-grow flex items-center justify-center w-full min-h-0">
            ...
            <div className="w-full flex flex-col gap-2.5 sm:gap-6 animate-fadeIn max-h-full">
              ...
              <div className={`p-4 bg-zinc-900/40 border border-zinc-800 rounded-2xl gap-3 ${
                isSidebarMinimized ? "flex" : "hidden lg:flex"
              } items-center`}>
        ...
        <aside className={`border-zinc-800 bg-[#121218] transition-all duration-300 flex flex-col flex-shrink-0 ${
          isSidebarMinimized 
          ...
          <div className="flex border-b border-zinc-800 p-3 bg-zinc-950/20 items-center justify-between gap-2 flex-shrink-0">
            <div className="flex flex-grow bg-zinc-900/40 p-1 rounded-xl">
              <button
                ...
                className={`flex-1 py-1.5 rounded-lg text-xs font-black flex items-center justify-center gap-1.5 transition ${
                  activeTab === "chat" 
                    ...
                    <div key={m.id} className="animate-fadeIn">
                      {m.isSystem ? (
                        ...
                        <div className="flex gap-2 sm:gap-3 items-start p-1.5 sm:p-2.5 rounded-xl hover:bg-zinc-900/40 transition group relative">
                          {/* Left: User Avatar */}
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs flex-shrink-0 select-none ${
                ...
                {participants.map((p, idx) => (
                  ...
                  >
                    ...
                    <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                      p.role === "Creator" ? "bg-red-600/10 text-red-400 border border-red-500/20" : "bg-zinc-800 text-zinc-400"
                    }`}>
                      {p.role}
                    </span>
                  </div>
